# 1
def apply_bonus(salary: int, 
                bonus_precent: float
) -> float:
    return salary * (1 + bonus_precent / 100)
...
def find_emp_name(emps: dict[int, str], 
                  emp_id: int
) -> Optional[str]:
...
def get_salary(data: Mapping[str, int], 
               name: str
) -> int:
    return data[name]
